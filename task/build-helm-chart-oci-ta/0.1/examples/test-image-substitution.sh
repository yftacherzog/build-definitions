...
# Test script to demonstrate image substitution functionality
...
cp test-chart-templates/values.yaml test-chart/
...
if [ "$IMAGE_MAPPINGS" != "[]" ]; then
    ...
    echo "$IMAGE_MAPPINGS" | jq -c '.[]' | while read -r mapping; do
        ...
        echo "Replacing '$source_image' with '$final_image' in templates..."
...
echo "3. localhost/init → quay.io/myorg/init:${standardized_tag}"
