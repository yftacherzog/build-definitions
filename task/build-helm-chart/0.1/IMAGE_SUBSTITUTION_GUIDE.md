# Image Substitution Guide for Build Helm Chart Task

## Overview

The enhanced `build-helm-chart` task now supports **image substitution** - the ability to replace placeholder images in Helm chart templates with actual built images before packaging. This feature is particularly useful in CI/CD pipelines where you want to:
...
Then use the `IMAGE_MAPPINGS` parameter to replace them:
```json
[
  {
...
Define the substitution mappings as JSON:
```json
[
  {
    "source": "localhost/myapp",
  ...
  - name: IMAGE_MAPPINGS
    value: |
      [
        {
          "source": "localhost/myapp",
          "target": "quay.io/myorg/myapp:v1.2.3"
        },
        {
          "source": "localhost/sidecar",
          "target": "quay.io/myorg/sidecar:latest"
        }
      ]
  workspaces:
  - name: source
...
Image substitution in the `build-helm-chart` task provides a simple, maintainable way to handle image references in Helm charts. By using placeholder images and hardcoded substitution mappings, you can:
