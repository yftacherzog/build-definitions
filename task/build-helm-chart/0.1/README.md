...
This Tekton task packages and pushes a Helm chart to an OCI repository with support for image substitution.
...
|-----------|-------------|---------|----------|
| `REPO` | Designated registry for the chart to be pushed to | - | Yes |
...
| `IMAGE_MAPPINGS` | JSON array of image mappings for substitution. Target images will have tags automatically set to VERSION_SUFFIX-COMMIT_SHA format. Substitutions occur in templates/ and values files. | `[]` | No |
...
[
  ...
  },
  {
    ...
    "target": "quay.io/myorg/another-app"
  }
]
```
...
1. **Source Images**: These are the placeholder images in your chart templates (e.g., `localhost/my/repo`)
2. **Target Images**: These are the registry/repository names without tags (e.g., `quay.io/myorg/myapp`)
3. **Automatic Tagging**: The task automatically appends the tag format: `VERSION_SUFFIX-COMMIT_SHA` (or just `COMMIT_SHA` if `VERSION_SUFFIX` is empty)
4. **Substitution**: The task finds all YAML files in the `templates/` directory and the specified values file, then replaces source images with target images (with standardized tags)
...
kind: TaskRun
metadata:
  name: build-helm-chart-with-custom-values
spec:
  taskRef:
    name: build-helm-chart
  params:
...
```yaml
image:
  repository: "quay.io/myorg/myapp:v1.2.3-abc123"
  pullPolicy: IfNotPresent
  tag: ""

sidecar:
  image:
    repository: "quay.io/myorg/sidecar:v1.2.3-abc123"
    pullPolicy: IfNotPresent
    tag: ""
```
...
- Image substitution affects files in the `templates/` directory and the specified values file
